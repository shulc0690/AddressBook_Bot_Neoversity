...
def validate_name(name):
    ...
    if re.fullmatch(r'[A-Za-zА-Яа-яЇїІіЄєҐґ]+', name):
        ...
        combed_msg = error_msg4return("Name and Last Name must contain only letters and be a single word.")
        raise ValueError(combed_msg)
...
def add_contact(args, book: AddressBook):
    if not args or not args[0].strip() or len(args) > 1:
        combed_msg = error_msg4return("Error: Name must be a single word without spaces. Please provide a valid name.") 
        return combed_msg
    name = args[0].strip()
    ...
    for i in range(2):
        combed_msg = info_msg4return("Enter phone number (10 digits) (or press Enter to skip): ")
    ...
    for i in range(2):
        combed_msg = info_msg4return("Enter birthday (DD.MM.YYYY) (or press Enter to skip): ")
...
# show all the contacts in the table
def print_contacts_table(book):
        table = Table(title="BUDANOV BLACK LIST", show_lines=True, title_style="green", border_style="green")
        ...
        for record in book.data.values():
            ...
            email_str = record.email.value if hasattr(record, 'email') and record.email else "No email"
            address_str = record.address.value if hasattr(record, 'address') and record.address else "No address"
            ...
            table.add_row(record.name.value, last_name_str, phones_str, email_str, address_str, birthday_str, notes_str)
...
def add_note_to_contact(args, book: AddressBook):
    ...
    tags = input(main_msg4return("Enter tags (separated by commas):")).strip().split(",")
...
def edit_note_in_contact(args, book: AddressBook):
    ...
    note_number = int(input(main_msg4return("Enter the number of the note you want to edit: ")).strip())
    if note_number < 1 or note_number > len(record.notes):
        combed_msg = info_msg4return("Invalid note number.")
        return combed_msg
    
    title = input(main_msg4return("Enter new note title (or press Enter to keep the current title): ")).strip()
    content = input(main_msg4return("Enter new note content (or press Enter to keep the current content): ")).strip()
    tags = input(main_msg4return("Enter new tags (separated by commas, or press Enter to keep current tags): ")).strip().split(',')
    if title:
...
def delete_note_from_contact(args, book: AddressBook):
    ...
    note_number = int(input(info_msg4return("Enter the number of the note you want to delete: ")).strip())
    if note_number < 1 or note_number > len(record.notes):
        combed_msg = info_msg4return("Invalid note number.")
        return combed_msg
        ...
        if choice == '1':
            ...
            for attempt in range(2):
                try:
                    ...
                    if attempt < 1:
                        error_msg(f"Error: {e}. You have {1 - attempt} attempt left.")
                        new_last_name = input(info_msg4return("Re-enter new last name: ")).strip()
                    else:
            ...
            while True:
                ...
                if phone_choice == '1':
                    ...
                    if len(record.phones) > 1:  # Якщо більше одного номера телефону
                        ...
                        phone_number_choice = int(input(main_msg4return("Enter the number of the phone you want to edit: ")).strip())
                        
                        if 1 <= phone_number_choice <= len(record.phones):
                            old_phone = record.phones[phone_number_choice - 1].value
                            for attempt in range(2):
                                new_phone = input(main_msg4return(f"Enter new phone number to replace {old_phone}: ")).strip()
                                try:
                    ...
                    for attempt in range(2):
                        new_phone = input(main_msg4return("Enter new phone number: ")).strip()
                        try:
                            validate_phone(new_phone)
            ...
            new_birthday = input(info_msg4return("Enter new birthday (DD.MM.YYYY): ")).strip()
...
def delete_phone(args, book):
    if len(args) < 2:
        return error_msg4return("Error: Please provide a contact name and the phone number to delete.")
